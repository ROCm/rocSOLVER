...
/* =====================================================================
   ...
   declaring prototype and calling. xlange and xaxpy prototype are like
   following
    =================================================================== */
...
float slansy_(char *norm_type, char *uplo, int *n, float *A, int *lda,
              float *work);
double dlansy_(char *norm_type, char *uplo, int *n, double *A, int *lda,
               double *work);
float clanhe_(char *norm_type, char *uplo, int *n, rocblas_float_complex *A,
              int *lda, float *work);
double zlanhe_(char *norm_type, char *uplo, int *n, rocblas_double_complex *A,
               int *lda, double *work);

void saxpy_(int *n, float *alpha, float *x, int *incx, float *y, int *incy);
void daxpy_(int *n, double *alpha, double *x, int *incx, double *y, int *incy);
void caxpy_(int *n, float *alpha, rocblas_float_complex *x, int *incx,
            rocblas_float_complex *y, int *incy);
void zaxpy_(int *n, double *alpha, rocblas_double_complex *x, int *incx,
 ...
 * float/double/complex template speciliazation
 * ======================================= */

/*! \brief compare the norm error of two matrices hCPU & hGPU */
template <>
double norm_check_general<rocblas_half>(char norm_type, rocblas_int M,
                                        rocblas_int N, rocblas_int lda,
                                        rocblas_half *hCPU,
                                        rocblas_half *hGPU) {
  // norm type can be O', 'I', 'F', 'o', 'i', 'f' for one, infinity or Frobenius
  // norm one norm is max column sum infinity norm is max row sum Frobenius is
  // l2 norm of matrix entries
  ...
  float error =
      slange_(&norm_type, &M, &N, hGPU_float.get(), &lda, &work) / cpu_norm;

  return static_cast<double>(error);
}

template <>
double norm_check_general<float>(char norm_type, rocblas_int M, rocblas_int N,
                                 rocblas_int lda, float *hCPU, float *hGPU) {
  // norm type can be O', 'I', 'F', 'o', 'i', 'f' for one, infinity or Frobenius
  // norm one norm is max column sum infinity norm is max row sum Frobenius is
  // l2 norm of matrix entries
...
template <>
double norm_check_general<double>(char norm_type, rocblas_int M, rocblas_int N,
                                  rocblas_int lda, double *hCPU, double *hGPU) {
  // norm type can be O', 'I', 'F', 'o', 'i', 'f' for one, infinity or Frobenius
  // norm one norm is max column sum infinity norm is max row sum Frobenius is
  // l2 norm of matrix entries
...
template <>
double norm_check_general<rocblas_float_complex>(char norm_type, rocblas_int M,
                                                 rocblas_int N, rocblas_int lda,
...
template <>
double norm_check_general<rocblas_double_complex>(
    char norm_type, rocblas_int M, rocblas_int N, rocblas_int lda,
...
/*! \brief compare the norm error of two hermitian/symmetric matrices hCPU &
 * hGPU */

template <>
double norm_check_symmetric<float>(char norm_type, char uplo, rocblas_int N,
                                   rocblas_int lda, float *hCPU, float *hGPU) {
...
template <>
double norm_check_symmetric<double>(char norm_type, char uplo, rocblas_int N,
                                    rocblas_int lda, double *hCPU,
                                    double *hGPU) {
  // norm type can be M', 'I', 'F', 'l': 'F' (Frobenius norm) is used mostly
...
template <>
double norm_check_symmetric<rocblas_float_complex>(
    char norm_type, char uplo, rocblas_int N, rocblas_int lda,
...
template <>
double norm_check_symmetric<rocblas_double_complex>(
    char norm_type, char uplo, rocblas_int N, rocblas_int lda,
    rocblas_double_complex *hCPU, rocblas_double_complex *hGPU) {
  // norm type can be M', 'I', 'F', 'l': 'F' (Frobenius norm) is used mostly
