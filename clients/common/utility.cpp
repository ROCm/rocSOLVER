...
template <> char type2char<float>() { return 's'; }

template <> char type2char<double>() { return 'd'; }

template <> char type2char<rocblas_float_complex>() { return 'c'; }

template <> char type2char<rocblas_double_complex>() { return 'z'; }
  ...
  for (rocblas_int i = 0; i < device_count; i++) {
    ...
    if (status != rocblas_status_success) {
      ...
      printf("Device ID %d : %s "
             "------------------------------------------------------\n",
             i, props.name);
      printf("with %3.1f GB memory, clock rate %dMHz @ computing capability "
             "%d.%d \n",
             props.totalGlobalMem / 1e9, (int)(props.clockRate / 1000),
             props.major, props.minor);
      printf("maxGridDimX %d, sharedMemPerBlock %3.1f KB, maxThreadsPerBlock "
             "%d, warpSize %d\n",
             props.maxGridSize[0], props.sharedMemPerBlock / 1e3,
...
void set_device(rocblas_int device_id) {
  ...
  if (status != rocblas_status_success) {
    printf("Set device error: cannot set device ID %d, there may not be such "
           "device ID\n",
           (int)device_id);
  }
}
