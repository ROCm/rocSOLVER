    ...
    for(rocblas_int b = 0; b < bc; ++b)
    {
        ...
        else
        {
            ...
            if(hInfo[b][0] == 0)
            {
                ...
                cpu_symm_hemm<T>(rocblas_side_left, uplo, n, hNev[b][0], alpha, B[b], ldb, hARes[b],
                                 lda, beta, hB[b], ldb);
        ...
        *cpu_time_used = get_time_us_no_sync();
        for(rocblas_int b = 0; b < bc; ++b)
        {
            cpu_sygvx_hegvx<T>(itype, evect, erange, uplo, n, hA[b], lda, hB[b], ldb, vl, vu, il,
                               iu, atol, hNev[b], hW[b], Z.data(), ldz, work.data(), lwork,
                               rwork.data(), iwork.data(), ifail.data(), hInfo[b]);
        }
