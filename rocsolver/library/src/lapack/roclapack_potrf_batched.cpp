                                            ...
                                            const rocblas_int batch_count)
{
    ...
    if(!handle)
        ROCSOLVER_RETURN_TOP("potrf_batched", rocblas_status_invalid_handle);

    // argument checking
    rocblas_status st = rocsolver_potf2_potrf_argCheck(handle, uplo, n, lda, A, info, batch_count);
    if(st != rocblas_status_continue)
        ROCSOLVER_RETURN_TOP("potrf_batched", st);
    ...
    if(!mem)
        ROCSOLVER_RETURN_TOP("potrf_batched", rocblas_status_memory_error);
    ...
    ROCSOLVER_RETURN_TOP(
        "potrf_batched",
        rocsolver_potrf_template<true, S, T>(handle, uplo, n, A, shiftA, lda, strideA, info,
