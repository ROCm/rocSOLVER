...
{
    ROCSOLVER_ENTER_TOP("geqlf_batched", "-m", m, "-n", n, "--lda", lda, "--bsp", stridep,
                        "--batch", batch_count);

    if(!handle)
        ROCSOLVER_RETURN_TOP("geqlf_batched", rocblas_status_invalid_handle);

    // argument checking
    rocblas_status st = rocsolver_geql2_geqlf_argCheck(handle, m, n, lda, A, ipiv, batch_count);
    if(st != rocblas_status_continue)
        ROCSOLVER_RETURN_TOP("geqlf_batched", st);
    ...
    if(rocblas_is_device_memory_size_query(handle))
        ROCSOLVER_RETURN_TOP("geqlf_batched",
    ...
    if(!mem)
        ROCSOLVER_RETURN_TOP("geqlf_batched", rocblas_status_memory_error);
    ...
    // execution
    ROCSOLVER_RETURN_TOP("geqlf_batched",
                         rocsolver_geqlf_template<true, false, T>(
