...
{
    ROCSOLVER_ENTER_TOP("gesvd_strided_batched", "--leftsv", left_svect, "--rightsv", right_svect,
                        "-m", m, "-n", n, "--lda", lda, "--bsa", strideA, "--bsb", strideS, "--ldb",
                        ldu, "--bsc", strideU, "--ldv", ldv, "--bsp", strideV, "--bs5", strideE,
                        "--workmode", fast_alg, "--batch", batch_count);

    if(!handle)
        ROCSOLVER_RETURN_TOP("gesvd_strided_batched", rocblas_status_invalid_handle);
    ...
    if(st != rocblas_status_continue)
        ROCSOLVER_RETURN_TOP("gesvd_strided_batched", st);
    ...
    if(rocblas_is_device_memory_size_query(handle))
        ROCSOLVER_RETURN_TOP("gesvd_strided_batched",
    ...
    if(!mem)
        ROCSOLVER_RETURN_TOP("gesvd_strided_batched", rocblas_status_memory_error);
    ...
    // execution
    ROCSOLVER_RETURN_TOP("gesvd_strided_batched",
                         rocsolver_gesvd_template<false, true, T>(
