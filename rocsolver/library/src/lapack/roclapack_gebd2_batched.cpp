                                            ...
                                            const rocblas_int batch_count)
{
    ...
    if(!handle)
        ROCSOLVER_RETURN_TOP("gebd2_batched", rocblas_status_invalid_handle);

    // argument checking
    rocblas_status st
        = rocsolver_gebd2_gebrd_argCheck(handle, m, n, lda, A, D, E, tauq, taup, batch_count);
    if(st != rocblas_status_continue)
        ROCSOLVER_RETURN_TOP("gebd2_batched", st);
    ...
    if(rocblas_is_device_memory_size_query(handle))
        ROCSOLVER_RETURN_TOP("gebd2_batched",
    ...
    if(!mem)
        ROCSOLVER_RETURN_TOP("gebd2_batched", rocblas_status_memory_error);
