...
{
    ROCSOLVER_ENTER_TOP("geqr2_strided_batched", "-m", m, "-n", n, "--lda", lda, "--bsa", strideA,
                        "--bsp", stridep, "--batch", batch_count);

    if(!handle)
        ROCSOLVER_RETURN_TOP("geqr2_strided_batched", rocblas_status_invalid_handle);

    // argument checking
    rocblas_status st = rocsolver_geqr2_geqrf_argCheck(handle, m, n, lda, A, ipiv, batch_count);
    if(st != rocblas_status_continue)
        ROCSOLVER_RETURN_TOP("geqr2_strided_batched", st);
    ...
    if(rocblas_is_device_memory_size_query(handle))
        ROCSOLVER_RETURN_TOP("geqr2_strided_batched",
    ...
    if(!mem)
        ROCSOLVER_RETURN_TOP("geqr2_strided_batched", rocblas_status_memory_error);
    ...
    // execution
    ROCSOLVER_RETURN_TOP("geqr2_strided_batched",
                         rocsolver_geqr2_template<T>(handle, m, n, A, shiftA, lda, strideA, ipiv,
