...
{
    ROCSOLVER_ENTER_TOP("gelqf_batched", "-m", m, "-n", n, "--lda", lda, "--bsp", stridep,
                        "--batch", batch_count);

    if(!handle)
        ROCSOLVER_RETURN_TOP("gelqf_batched", rocblas_status_invalid_handle);

    // argument checking
    rocblas_status st = rocsolver_gelq2_gelqf_argCheck(handle, m, n, lda, A, ipiv, batch_count);
    if(st != rocblas_status_continue)
        ROCSOLVER_RETURN_TOP("gelqf_batched", st);
    ...
    if(rocblas_is_device_memory_size_query(handle))
        ROCSOLVER_RETURN_TOP("gelqf_batched",
    ...
    if(!mem)
        ROCSOLVER_RETURN_TOP("gelqf_batched", rocblas_status_memory_error);
    ...
    // execution
    ROCSOLVER_RETURN_TOP("gelqf_batched",
                         rocsolver_gelqf_template<true, false, T>(
...
rocblas_status rocsolver_cgelqf_batched(rocblas_handle handle,
                                        ...
                                        const rocblas_int batch_count)
{
    return rocsolver_gelqf_batched_impl<rocblas_float_complex>(handle, m, n, A, lda, ipiv, stridep,
                                                               batch_count);
}
...
rocblas_status rocsolver_zgelqf_batched(rocblas_handle handle,
                                        ...
                                        const rocblas_int batch_count)
{
    return rocsolver_gelqf_batched_impl<rocblas_double_complex>(handle, m, n, A, lda, ipiv, stridep,
                                                                batch_count);
}
