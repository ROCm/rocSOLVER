...
rocblas_status rocsolver_gelqf_impl(rocblas_handle handle,
                                    ...
                                    T* ipiv)
{
    ROCSOLVER_ENTER_TOP("gelqf", "-m", m, "-n", n, "--lda", lda);

    if(!handle)
        ROCSOLVER_RETURN_TOP("gelqf", rocblas_status_invalid_handle);

    // argument checking
    rocblas_status st = rocsolver_gelq2_gelqf_argCheck(handle, m, n, lda, A, ipiv);
    if(st != rocblas_status_continue)
        ROCSOLVER_RETURN_TOP("gelqf", st);
    ...
    if(!mem)
        ROCSOLVER_RETURN_TOP("gelqf", rocblas_status_memory_error);
    ...
    // execution
    ROCSOLVER_RETURN_TOP("gelqf",
                         rocsolver_gelqf_template<false, false, T>(
