                                            ...
                                            const rocblas_int batch_count)
{
    ...
    if(!handle)
        ROCSOLVER_RETURN_TOP("geqrf_batched", rocblas_status_invalid_handle);

    // argument checking
    rocblas_status st = rocsolver_geqr2_geqrf_argCheck(handle, m, n, lda, A, ipiv, batch_count);
    if(st != rocblas_status_continue)
        ROCSOLVER_RETURN_TOP("geqrf_batched", st);
    ...
    if(rocblas_is_device_memory_size_query(handle))
        ROCSOLVER_RETURN_TOP("geqrf_batched",
    ...
    if(!mem)
        ROCSOLVER_RETURN_TOP("geqrf_batched", rocblas_status_memory_error);
    ...
    // execution
    ROCSOLVER_RETURN_TOP("geqrf_batched",
                         rocsolver_geqrf_template<true, false, T>(
...
rocblas_status rocsolver_cgeqrf_batched(rocblas_handle handle,
                                        ...
                                        const rocblas_int batch_count)
{
    return rocsolver_geqrf_batched_impl<rocblas_float_complex>(handle, m, n, A, lda, ipiv, stridep,
                                                               batch_count);
}
...
rocblas_status rocsolver_zgeqrf_batched(rocblas_handle handle,
                                        ...
                                        const rocblas_int batch_count)
{
    return rocsolver_geqrf_batched_impl<rocblas_double_complex>(handle, m, n, A, lda, ipiv, stridep,
                                                                batch_count);
}
