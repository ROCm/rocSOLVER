...
{
    ROCSOLVER_ENTER_TOP("geql2_batched", "-m", m, "-n", n, "--lda", lda, "--bsp", stridep,
                        "--batch", batch_count);

    if(!handle)
        ROCSOLVER_RETURN_TOP("geql2_batched", rocblas_status_invalid_handle);
    ...
    if(st != rocblas_status_continue)
        ROCSOLVER_RETURN_TOP("geql2_batched", st);
    ...
    if(rocblas_is_device_memory_size_query(handle))
        ROCSOLVER_RETURN_TOP("geql2_batched",
    ...
    if(!mem)
        ROCSOLVER_RETURN_TOP("geql2_batched", rocblas_status_memory_error);
...
rocblas_status rocsolver_cgeql2_batched(rocblas_handle handle,
                                        ...
                                        const rocblas_int batch_count)
{
    return rocsolver_geql2_batched_impl<rocblas_float_complex>(handle, m, n, A, lda, ipiv, stridep,
                                                               batch_count);
}
...
rocblas_status rocsolver_zgeql2_batched(rocblas_handle handle,
                                        ...
                                        const rocblas_int batch_count)
{
    return rocsolver_geql2_batched_impl<rocblas_double_complex>(handle, m, n, A, lda, ipiv, stridep,
                                                                batch_count);
}
