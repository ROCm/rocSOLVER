  ...
  if (uplo ==
    ...
    for (rocblas_int j = 0; j < n; j += POTRF_POTF2_SWITCHSIZE) {
      ...
      if (j + jb < n) {
        ...
        for (int b = 0; b < batch_count; ++b) {
          ...
          rocblas_trsm(handle, rocblas_side_left, uplo,
                       rocblas_operation_conjugate_transpose,
                       rocblas_diagonal_non_unit, jb, (n - j - jb), &t_one,
                       (M + idx2D(j, j, lda)), lda, (M + idx2D(j, j + jb, lda)),
                       lda);
        }
    ...
    for (rocblas_int j = 0; j < n; j += POTRF_POTF2_SWITCHSIZE) {
      ...
      if (j + jb < n) {
        ...
        for (int b = 0; b < batch_count; ++b) {
          M = load_ptr_batch<T>(AA, b, shiftA, strideA);
          rocblas_trsm(handle, rocblas_side_right, uplo,
                       rocblas_operation_conjugate_transpose,
                       rocblas_diagonal_non_unit, (n - j - jb), jb, &t_one,
                       (M + idx2D(j, j, lda)), lda, (M + idx2D(j + jb, j, lda)),
                       lda);
        }
