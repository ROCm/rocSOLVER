                                    ...
                                    T* ipiv)
{
    ...
    if(!handle)
        ROCSOLVER_RETURN_TOP("geql2", rocblas_status_invalid_handle);

    // argument checking
    rocblas_status st = rocsolver_geql2_geqlf_argCheck(handle, m, n, lda, A, ipiv);
    if(st != rocblas_status_continue)
        ROCSOLVER_RETURN_TOP("geql2", st);
    ...
    if(!mem)
        ROCSOLVER_RETURN_TOP("geql2", rocblas_status_memory_error);
