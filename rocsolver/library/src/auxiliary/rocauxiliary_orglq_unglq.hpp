  ...
  while (j >= 0) {
    ...
    if (j + jb < m) {
      ...
      rocsolver_larfb_template<BATCHED, STRIDED, T>(
          ...
          shiftA + idx2D(j + jb, j, lda), lda, strideA, batch_count, workTrmm,
          work, workArr);
    }
