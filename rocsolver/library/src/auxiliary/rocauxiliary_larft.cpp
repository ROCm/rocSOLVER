...
template <typename T>
rocblas_status
rocsolver_larft_impl(rocblas_handle handle, const rocblas_direct direct,
  ...
  // argument checking
  rocblas_status st =
      rocsolver_larft_argCheck(direct, storev, n, k, ldv, ldf, V, tau, F);
  ...
  rocsolver_larft_getMemorySize<T, false>(k, batch_count, &size_1, &size_2,
                                          &size_3);
  ...
  // execution
  rocblas_status status = rocsolver_larft_template<T>(
      handle, direct, storev, n, k, V, 0, // shifted 0 entries
      ldv, stridev, tau, stridet, F, ldf, stridef, batch_count, (T *)scalars,
      (T *)work, (T **)workArr);
...
rocsolver_zlarft(rocblas_handle handle, const rocblas_direct direct,
  ...
  return rocsolver_larft_impl<rocblas_double_complex>(handle, direct, storev, n,
                                                      k, V, ldv, tau, T, ldt);
}
