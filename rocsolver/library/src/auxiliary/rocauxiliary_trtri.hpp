...
template <typename T>
__device__ void trtri_check_singularity(const rocblas_int n, T *a,
...
template <typename T>
__device__ void trtri_unblk(const rocblas_diagonal diag, const rocblas_int n,
                            T *a, const rocblas_int lda, rocblas_int *info,
                            T *w) {
...
__global__ void
trtri_kernel(const rocblas_diagonal diag, const rocblas_int n, U A,
             const rocblas_int shiftA, const rocblas_int lda,
             const rocblas_stride strideA, rocblas_int *info, V work) {
  int b = hipBlockIdx_x;
  ...
  if (n <= TRTRI_SWITCHSIZE_MID)
    ...
    for (rocblas_int j = 0; j < n; j += nb) {
      ...
      trsm_kernel_right_upper(diag, j, jb, &minone, a + j + j * lda, lda,
                              a + j * lda, lda);
...
template <typename T, typename U, typename V>
__global__ void
trtri_kernel_large(const rocblas_diagonal diag, const rocblas_int n,
                   const rocblas_int j, const rocblas_int jb, U A,
                   const rocblas_int shiftA, const rocblas_int lda,
                   const rocblas_stride strideA, rocblas_int *info, V work) {
  int b = hipBlockIdx_x;
...
void rocsolver_trtri_getMemorySize(const rocblas_int n,
...
template <typename T>
rocblas_status
rocsolver_trtri_argCheck(const rocblas_int n, const rocblas_int lda, T A,
...
rocblas_status rocsolver_trtri_template(
    rocblas_handle handle, const rocblas_fill uplo, const rocblas_diagonal diag,
  ...
  if (n <= TRTRI_SWITCHSIZE_LARGE) {
    hipLaunchKernelGGL(trtri_kernel<T>, dim3(batch_count, 1, 1),
                       dim3(1, threads, 1), 0, stream, diag, n, A, shiftA, lda,
                       strideA, info, work);
  } else {
    ...
    for (rocblas_int j = 0; j < n; j += nb) {
      ...
      rocblasCall_trmm<BATCHED, STRIDED, T>(
          ...
          diag, j, jb, &one, A, shiftA, lda, strideA, A,
          shiftA + idx2D(0, j, lda), lda, strideA, batch_count, work + shiftW,
          workArr);

      hipLaunchKernelGGL(trtri_kernel_large<T>, dim3(batch_count, 1, 1),
                         dim3(1, threads, 1), 0, stream, diag, n, j, jb, A,
                         shiftA, lda, strideA, info, work);
    }
