                                   ...
                                   const rocblas_int k, T* A, const rocblas_int lda, T* ipiv, T *C, const rocblas_int ldc)
{
    ...
    // argument checking
    rocblas_status st = rocsolver_orm2r_ormqr_argCheck(side,trans,m,n,k,lda,ldc,A,C,ipiv);
    ...
    rocsolver_ormqr_getMemorySize<T,false>(side,m,n,k,batch_count,&size_1,&size_2,&size_3,&size_4);
    ...
    rocblas_status status = 
           rocsolver_ormqr_template<false,false,T>(handle,side,trans,
                                                  m,n,k,
