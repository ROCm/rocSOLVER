...
/*! \brief GEQRF_PTR_BATCHED computes the QR factorization of a batch of general m-by-n matrices.
    ...
    lda       rocblas_int. lda >= m.\n
              Specifies the leading dimension of matrices A_j. 
    @param[out]
    ipiv      Array of pointers to type. Each pointer points to an array on the GPU of dimension min(m,n).\n
              Contains the vectors ipiv_j of scalar factors of the 
              Householder matrices H_j(i).
    @param[in]
    batch_count  rocblas_int. batch_count >= 0.\n
