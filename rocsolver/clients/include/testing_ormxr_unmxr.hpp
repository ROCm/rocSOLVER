...
{
    // handle
    EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,nullptr,side,trans,m,n,k,dA,lda,dIpiv,dC,ldc),
                          rocblas_status_invalid_handle); 

    // values
    EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,rocblas_side(-1),trans,m,n,k,dA,lda,dIpiv,dC,ldc),
                          rocblas_status_invalid_value);
    EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,side,rocblas_operation(-1),m,n,k,dA,lda,dIpiv,dC,ldc),
                          rocblas_status_invalid_value); 
    if (COMPLEX)
        EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,side,rocblas_operation_transpose,m,n,k,dA,lda,dIpiv,dC,ldc),
                              rocblas_status_invalid_value); 
    else
        EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,side,rocblas_operation_conjugate_transpose,m,n,k,dA,lda,dIpiv,dC,ldc),
                              rocblas_status_invalid_value); 
    
    // pointers
    EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,side,trans,m,n,k,(T)nullptr,lda,dIpiv,dC,ldc),
                          rocblas_status_invalid_pointer);
    EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,side,trans,m,n,k,dA,lda,(T)nullptr,dC,ldc),
                          rocblas_status_invalid_pointer);
    EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,side,trans,m,n,k,dA,lda,dIpiv,(T)nullptr,ldc),
                          rocblas_status_invalid_pointer);

    // quick return with invalid pointers
    EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,rocblas_side_right,trans,0,n,k,dA,lda,dIpiv,(T)nullptr,ldc),
                          rocblas_status_success);
    EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,rocblas_side_left,trans,m,0,k,dA,lda,dIpiv,(T)nullptr,ldc),
                          rocblas_status_success);
    EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,rocblas_side_left,trans,m,n,0,(T)nullptr,lda,(T)nullptr,dC,ldc),
                          rocblas_status_success);
}
...
void testing_ormxr_unmxr_bad_arg()
{
    ...
    // check bad arguments
    ormxr_unmxr_checkBadArgs<MQR,COMPLEX>(handle,side,trans,m,n,k,dA.data(),lda,dIpiv.data(),dC.data(),ldc);
}   
...
void testing_ormxr_unmxr(Arguments argus) 
{
    ...
    if (invalid_value) {
        EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR, handle, side, trans, m, n, k, (T*)nullptr, lda, (T*)nullptr, (T*)nullptr, ldc),
                              rocblas_status_invalid_value);
    ...
    // check quick return
    if (n == 0 || m == 0 || k == 0) {
        EXPECT_ROCBLAS_STATUS(rocsolver_ormxr_unmxr(MQR,handle,side,trans,m,n,k,dA.data(),lda,dIpiv.data(),dC.data(),ldc),
                              rocblas_status_success);
    ...
    if (argus.unit_check || argus.norm_check)
        ormxr_unmxr_getError<MQR,T>(handle, side, trans, m, n, k, dA, lda, dIpiv, dC, ldc,
                         hA, hIpiv, hC, hCr, &max_error); 

    // collect performance data 
    if (argus.timing) 
        ormxr_unmxr_getPerfData<MQR,T>(handle, side, trans, m, n, k, dA, lda, dIpiv, dC, ldc,
