...
rocblas_status testing_geqr2_geqrf_strided_batched(Arguments argus) {
    ...
    if (M < 1 || N < 1 || lda < M || batch_count < 0) {
        ...
        if(geqrf) {
            return rocsolver_geqrf_strided_batched<T>(handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count);
        }
        else { 
            return rocsolver_geqr2_strided_batched<T>(handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count);
        }
    }
...
/* =====================================================================
    ...
    if (argus.unit_check || argus.norm_check) {
        //GPU lapack
        if(geqrf) {
            CHECK_ROCBLAS_ERROR(rocsolver_geqrf_strided_batched<T>(handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count));
        }
        else {
            CHECK_ROCBLAS_ERROR(rocsolver_geqr2_strided_batched<T>(handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count));
        }
        ...
        if(geqrf) {
            for (int b = 0; b < batch_count; ++b) 
                cblas_geqrf<T>(M, N, (hA.data() + b*strideA), lda, (hIpiv.data() + b*stridep), hw.data(), N);
        }
        else {
            for (int b = 0; b < batch_count; ++b) 
                cblas_geqr2<T>(M, N, (hA.data() + b*strideA), lda, (hIpiv.data() + b*stridep), hw.data());
        }
    ...
    if (argus.timing) {
        ...
        if(geqrf) {
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_geqrf_strided_batched<T>(handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count);
            ...
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_geqr2_strided_batched<T>(handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count);
            gpu_time_used = get_time_us();
            for(int iter = 0; iter < hot_calls; iter++)
                rocsolver_geqr2_strided_batched<T>(handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count);
            gpu_time_used = (get_time_us() - gpu_time_used) / hot_calls;
