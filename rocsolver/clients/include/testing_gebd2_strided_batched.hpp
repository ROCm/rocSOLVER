...
rocblas_status testing_gebd2_strided_batched(Arguments argus) {
    ...
    if (M < 1 || N < 1 || lda < M || batch_count < 0) {
        ...
        return rocsolver_gebd2_strided_batched(handle, M, N, dA, lda, strideA,
...
/* =====================================================================
    ...
    if (argus.unit_check || argus.norm_check) {
        //GPU lapack
        CHECK_ROCBLAS_ERROR(rocsolver_gebd2_strided_batched(handle, M, N, dA, lda, strideA,
            dD, strideP, dE, strideP, dTauq, strideP, dTaup, strideP, batch_count));
        ...
        for (int b = 0; b < batch_count; ++b)
        {
            cblas_gebd2<U,T>(M, N, (hA.data() + b*strideA), lda, (hD.data() + b*strideP), (hE.data() + b*strideP),
    ...
    if (argus.timing) {
        ...
        {
            rocsolver_gebd2_strided_batched(handle, M, N, dA, lda, strideA,
                dD, strideP, dE, strideP, dTauq, strideP, dTaup, strideP, batch_count);
        }
