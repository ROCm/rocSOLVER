                        ...
                        double *max_err)
{
    ...
    labrd_initData<true,true,S,T>(handle, m, n, nb, dA, lda, dD, dE, dTauq, dTaup, dX, ldx, dY, ldy,
                            ...
                            const bool perf)
{
    ...
    {
        labrd_initData<false,true,S,T>(handle, m, n, nb, dA, lda, dD, dE, dTauq, dTaup, dX, ldx, dY, ldy,
                            hA, hD, hE, hTauq, hTaup, hX, hY);
    ...
    for(rocblas_int iter = 0; iter < hot_calls; iter++)
    {
        ...
        start = get_time_us();
        rocsolver_labrd(handle, m, n, nb, dA.data(), lda, dD.data(), dE.data(), dTauq.data(), dTaup.data(), dX.data(), ldx, dY.data(), ldy);
...
void testing_labrd(Arguments argus) 
{
    ...
    if (size_Q) CHECK_HIP_ERROR(dTauq.memcheck());
    if (size_P) CHECK_HIP_ERROR(dTaup.memcheck());
    if (size_X) CHECK_HIP_ERROR(dX.memcheck());
    if (size_Y) CHECK_HIP_ERROR(dY.memcheck());

    // check quick return
    if (m == 0 || n == 0 || nb == 0) {
        EXPECT_ROCBLAS_STATUS(rocsolver_labrd(handle, m, n, nb, dA.data(), lda, dD.data(), dE.data(), dTauq.data(), dTaup.data(), dX.data(), ldx, dY.data(), ldy),
                              rocblas_status_success);
        if (argus.timing)
