                            ...
                            const bool perf)
{
    ...
    for(rocblas_int iter = 0; iter < hot_calls; iter++)
    {
        ...
        start = get_time_us();
        rocsolver_geql2_geqlf(STRIDED,GEQLF,handle, m, n, dA.data(), lda, stA, dIpiv.data(), stP, bc);
...
void testing_geql2_geqlf(Arguments argus) 
{
    ...
    if (invalid_size) {
        if (BATCHED)
            EXPECT_ROCBLAS_STATUS(rocsolver_geql2_geqlf(STRIDED,GEQLF,handle, m, n, (T *const *)nullptr, lda, stA, (T*)nullptr, stP, bc),
                                  rocblas_status_invalid_size);
        else
    ...
    if (BATCHED) {
        ...
        // check quick return
        if (m == 0 || n == 0 || bc == 0) {
            EXPECT_ROCBLAS_STATUS(rocsolver_geql2_geqlf(STRIDED,GEQLF,handle, m, n, dA.data(), lda, stA, dIpiv.data(), stP, bc),
                                  rocblas_status_success);
            if (argus.timing)
