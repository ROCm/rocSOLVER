...
rocblas_status testing_potf2_potrf_batched(Arguments argus) {
    ...
    if (N < 1 || lda < N || batch_count < 1) {
        ...
        if (potrf)
            return rocsolver_potrf_batched<T>(handle, uplo, N, dA, lda, dinfo, batch_count);
        else
            return rocsolver_potf2_batched<T>(handle, uplo, N, dA, lda, dinfo, batch_count);
    }

    // Naming: dK is in GPU (device) memory. hK is in CPU (host) memory
    vector<T> hA[batch_count];
    vector<T> AAT[batch_count];
    for (int b = 0; b < batch_count; ++b) {
        hA[b] = vector<T>(size_A);
        AAT[b] = vector<T>(size_A);
    }
    ...
    if (argus.unit_check || argus.norm_check) {
        // GPU lapack
        if (potrf) {
            CHECK_ROCBLAS_ERROR(rocsolver_potrf_batched<T>(handle, uplo, N, dA, lda, dinfo, batch_count));
        } else {
            CHECK_ROCBLAS_ERROR(rocsolver_potf2_batched<T>(handle, uplo, N, dA, lda, dinfo, batch_count));
        }

        //copy result to cpu
        for (int b = 0; b < batch_count; ++b) 
            CHECK_HIP_ERROR(hipMemcpy(AAT[b].data(), A[b], sizeof(T) * size_A, hipMemcpyDeviceToHost));
        ...
        if (potrf) {
            for(int b = 0; b < batch_count; ++b)
                cblas_potrf<T>(uplo, N, hA[b].data(), lda, (hinfo.data() + b));
        } else {
            for(int b = 0; b < batch_count; ++b)
                cblas_potf2<T>(uplo, N, hA[b].data(), lda, (hinfo.data() + b));
        }
    ...
    if (argus.timing) {
        ...
        if (potrf) {
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_potrf_batched<T>(handle, uplo, N, dA, lda, dinfo, batch_count);
            ...
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_potf2_batched<T>(handle, uplo, N, dA, lda, dinfo, batch_count);
            gpu_time_used = get_time_us(); // in microseconds
            for(int iter = 0; iter < hot_calls; iter++)
                rocsolver_potf2_batched<T>(handle, uplo, N, dA, lda, dinfo, batch_count);
            gpu_time_used = get_time_us() - gpu_time_used;
        ...
        cout << "N , lda , uplo , batch , us [gpu] , us [cpu]";

        if (argus.norm_check)
            cout << " , norm_error_host_ptr";

        cout << endl;
        cout << N << " , " << lda << " , " << char_uplo << " , " << batch_count << " , " << gpu_time_used
             << " , " << cpu_time_used;
