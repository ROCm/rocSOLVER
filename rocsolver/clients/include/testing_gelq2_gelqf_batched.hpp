...
rocblas_status testing_gelq2_gelqf_batched(Arguments argus) {
    ...
    if (M < 1 || N < 1 || lda < M || batch_count < 0) {
        ...
        if(gelqf) {
            return rocsolver_gelqf_batched<T>(handle, M, N, dA, lda, dIpiv, stridep, batch_count);
        }
        else { 
            return rocsolver_gelq2_batched<T>(handle, M, N, dA, lda, dIpiv, stridep, batch_count);
        }
    }
...
/* =====================================================================
    ...
    if (argus.unit_check || argus.norm_check) {
        //GPU lapack
        if(gelqf) {
            CHECK_ROCBLAS_ERROR(rocsolver_gelqf_batched<T>(handle, M, N, dA, lda, dIpiv, stridep, batch_count));
        }
        else {
            CHECK_ROCBLAS_ERROR(rocsolver_gelq2_batched<T>(handle, M, N, dA, lda, dIpiv, stridep, batch_count));
        }
        ...
        if(gelqf) {
            for (int b = 0; b < batch_count; ++b) 
                cblas_gelqf<T>(M, N, hA[b].data(), lda, (hIpiv.data() + b*stridep), hw.data(), M);
        }
        else {
            for (int b = 0; b < batch_count; ++b) 
                cblas_gelq2<T>(M, N, hA[b].data(), lda, (hIpiv.data() + b*stridep), hw.data());
        }
    ...
    if (argus.timing) {
        ...
        if(gelqf) {
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_gelqf_batched<T>(handle, M, N, dA, lda, dIpiv, stridep, batch_count);
            ...
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_gelq2_batched<T>(handle, M, N, dA, lda, dIpiv, stridep, batch_count);
            gpu_time_used = get_time_us();
            for(int iter = 0; iter < hot_calls; iter++)
                rocsolver_gelq2_batched<T>(handle, M, N, dA, lda, dIpiv, stridep, batch_count);
            gpu_time_used = (get_time_us() - gpu_time_used) / hot_calls;
