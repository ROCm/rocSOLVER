...
rocblas_status testing_gelq2_gelqf(Arguments argus) {
    ...
    if (M < 1 || N < 1 || lda < M) {
        ...
        if(gelqf) {
            return rocsolver_gelqf<T>(handle, M, N, dA, lda, dIpiv);
        }
        else { 
            return rocsolver_gelq2<T>(handle, M, N, dA, lda, dIpiv);
        }
    }
...
/* =====================================================================
    ...
    if (argus.unit_check || argus.norm_check) {
        //GPU lapack
        if(gelqf) {
            CHECK_ROCBLAS_ERROR(rocsolver_gelqf<T>(handle, M, N, dA, lda, dIpiv));
        }
        else {
            CHECK_ROCBLAS_ERROR(rocsolver_gelq2<T>(handle, M, N, dA, lda, dIpiv));
        }   
    ...
    if (argus.timing) {
        ...
        if(gelqf) {
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_gelqf<T>(handle, M, N, dA, lda, dIpiv);
            ...
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_gelq2<T>(handle, M, N, dA, lda, dIpiv);
            gpu_time_used = get_time_us();
            for(int iter = 0; iter < hot_calls; iter++)
                rocsolver_gelq2<T>(handle, M, N, dA, lda, dIpiv);
            gpu_time_used = (get_time_us() - gpu_time_used) / hot_calls;       
